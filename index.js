...
function outGoingMessage() {
  ...
  //Simulate bot response with thinking indicator ater delay
  setTimeout(() => {
    const botThinkingHtml = `
      <div class="message bot-message thinking">
        <svg class="bot-avatar" width="64px" height="64px" viewBox="0 -19.5 164 164" fill="none"
  ...
  `;
    ...
    document.querySelector(".chat-body").innerHTML += botThinkingHtml;
    generateBotResponse();
  ...
  //reset file-upload image indicator
  fileUploadWrapper.classList.remove("file-uploaded");
...
async function generateBotResponse() {
  ...
  try {
    const res = await fetch(API_URL + API_KEY, requestOptions);
    const message = await res.json();
    const botText = message.candidates[0].content.parts[0].text
      .replace(/\*+/g, "")
      .trim();
    ...
    inComingBotMessage(
      `<span style="color: #ff0000;">Sorry, an error occurred: ${escapeHtml(
        error.message
      )}</span>`,
      true
    );
...
function inComingBotMessage(botText, isHtml = false) {
  const thinkingMsg = document.querySelector(".bot-message.thinking");
  ...
  const botResponseHtml = `
      <div class="message bot-message">
        ...
        <div class="message-text">
          ${isHtml ? botText : escapeHtml(botText)}
          </div>
        </div>
      </div>
  `;
  ...
  document
    .querySelector(".chat-body")
    .insertAdjacentHTML("beforeend", botResponseHtml);
...
const picker = new EmojiMart.Picker({
  ...
  onClickOutside: (e) => {
    // Check if the click happened on the emoji picker button or one of its children.
    // Using .closest() ensures it works even if you click an icon inside the button.
    if (e.target.closest("#emoji-picker")) {
      // If button clicked, toggle the emoji picker's visibility.
      document.body.classList.toggle("show-emoji-picker");
    } else {
      // If the click was anywhere else outside the picker then hide it.
